...
local base_pipe_names = { "pipe" }
if script.active_mods["IndustrialRevolution3"] then
    table.insert(base_pipe_names, "copper-pipe")
    table.insert(base_pipe_names, "steam-pipe")
    table.insert(base_pipe_names, "air-pipe")
end
...
function pipe_utils.is_flowing(pipe, dir)
  local dirpos = util.direction_vectors[dir]
...
function pipe_utils.is_aligned(groundpipe, dir)
  return dir == groundpipe.direction or dir == pipeutil.opposite[groundpipe.direction]
end

function pipe_utils.is_blocked(pipe, dir, ignore_close)
  local dirpos = util.direction_vectors[dir]
  local searchpos = math2d.position.add(pipe.position, dirpos)
  
  -- search for all neighboring entities at the search position
  local others = pipe.surface.find_entities({
    math2d.position.add(searchpos,{-0.4, -0.4}),
    math2d.position.add(searchpos,{ 0.4,  0.4})})
  if #others > 0 then
...
function pipe_utils.get_juncname(directions)
  local juncname = ""
  if (directions[defines.direction.north]) then juncname = juncname.."n" end
  if (directions[defines.direction.east]) then juncname = juncname.."e" end
  if (directions[defines.direction.south]) then juncname = juncname.."s" end
  if (directions[defines.direction.west]) then juncname = juncname.."w" end
...
function pipe_utils.try_unlock_pipe(player, pipe, check_fluid_compatibility)
  ...
  for _,dir in pairs(pipeutil.directions) do
    if states[dir] == "close" then
      if (not check_fluid_compatibility) or (not pipe_utils.is_blocked(pipe, dir, true)) then
        states.directions[dir] = true
        do_unlock = true
      end
    end
...
{
  [2] = defines.direction.north,
  [4] = defines.direction.west,
  [6] = defines.direction.east,
